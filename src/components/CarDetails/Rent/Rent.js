        ...
        if (this.props.fromDate !== "" && this.props.toDate !== "") {
            ...
            if (this.props.fromDate <= this.props.toDate) {
                ...
                if (this.props.fromDate >= date) {
                    ...
                    this.props.onRentHandlerError("From field shouldn't be less than today's date!!!")
    
                }
            } else {
                this.props.onRentHandlerError("From field should be less than or equal To Field")
            }
    ...
    render() {
        ...
        return (
            <div>
                ...
                <button
                    style={{
                        borderRadius: "10px", float: "right", backgroundColor: "black",
                        outline: "none", width: '100px', textDecoration: "underline", color: "white"
                    }}
                    onClick={() => this.props.onRentToggleModal()}>RENT</button>

                <Modal isOpen={this.props.rentModalIsOpen}
                    className="modal-dialog-centered" toggle={() => this.props.onRentToggleModal()}>
                    <ModalHeader toggle={() => this.props.onRentToggleModal()}
                        style={{ borderBottom: "0.5px solid rgba(169, 169, 169, .5)" }}
                        cssModule={{ 'modal-title': 'w-100 text-center' }}
                        className={classes.modalHeader}>

                        <p className="modal-title" style={{ fontSize: "30px", textDecoration: "underline" }}>Rent a Car</p>
                    ...
                    <ModalBody className="container">
                        ...
                        <form className="form-inline ">
                            ...
                            <input type="date" name="FromDate" max="3000-12-31"
                                ...
                                style={{
                                    border: "2px solid black", borderRadius: "10px", padding: "3px",
                        ...
                        <form className="form-inline " >
                            <input type="date" name="ToDate" max="3000-12-31"
                                ...
                                className={classes.dateWidth}
                                onChange={(event) => this.props.onToDate(event, this.props.pricePerday)}
                                disabled={this.props.toDateDisabled}
                            />
                        </form>

                        <div style={{ display: "block" }} className={classes.floatTotalPrice}>
                            <span><strong>Total Cost:</strong> </span>
                            <label
                                className="rounded"
                                style={{ backgroundColor: "black", color: "white", textAlign: "center", border: "1px solid black" }}>
                    ...
                    <ModalFooter>
                        ...
                        <Button color="danger" onClick={this.rentHandler} >Rent</Button>
                    </ModalFooter>

                </Modal>

                {/*///////////////////// */}
                <Modal isOpen={this.state.confirmationModalIsOpen && confirmationModalIsOpen}

                    className="modal-dialog-centered" toggle={this.confirmationToggleModal}>
                    ...
                    <ModalFooter>
                        ...
                        <Button color="success" onClick={this.confirmHandler} disabled={this.props.isLoading}>
                            {this.props.isLoading && <i className="spinner-border spinner-border-sm" role="status"></i>}
...
const mapDispatchToProps = dispatch => {
    return {
        ...
        onToDate: (event, pricePerday) => dispatch(actions.toDateOnChangeHandler(event, pricePerday)),

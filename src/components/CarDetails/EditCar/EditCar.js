...
class EditCar extends Component {
  ...
  editHandler = () => {
    ...
    if (
      this.props.carNameEdit !== this.props.carName ||
      this.state.pricePerdayEdit !== this.state.pricePerday ||
      this.state.bodyTypeEdit !== this.state.bodyType ||
      this.state.classEdit !== this.state.class ||
      this.props.engineEdit !== this.props.engine ||
      this.props.horsePowerEdit !== this.props.horsePower ||
      image !== undefined ||
      this.state.interiorImagesArrayLength !==
        this.state.interiorImagesArray.length ||
      this.props.exteriorImagesFileNameLength !==
        this.props.exteriorImagesFileName.length ||
      this.state.exteriorImagesFile.length > 0 ||
      this.state.interiorImagesFile.length > 0
    ) {
      ...
      if (
        this.props.interiorImagesFileNameLength !==
        this.props.interiorImagesFileName.length
      ) {
        this.props.onSetNewInteriorImagesFileNameLength(
          this.props.interiorImagesFileName.length
        )
        ...
        this.props.onUpdateInteriorImagesArray(
          this.props.interiorImagesFileName
        )
      }
      ...
      if (
        ...
        this.props.onSetNewExteriorImagesFileNameLength(
          this.props.exteriorImagesFileName.length
        )
        ...
        // })
        this.props.onUpdateExteriorImagesArray(
          this.props.exteriorImagesFileName
        )
      }

      console.log(
        this.state.exteriorImagesFileFirebase.length,
        ' this.state.exteriorImagesFileFirebase.length'
      )
      if (this.state.exteriorImagesFile.length > 0) {
        ...
        if (i < this.state.exteriorImagesFile.length) {
          if (this.state.exteriorImagesFileFirebase.length !== 0) {
            for (
              let i = 0;
              i < this.state.exteriorImagesFileFirebase.length;
              i++
            ) {
              if (this.state.exteriorImagesFileFirebase[i] !== '') {
                this.state.exteriorImages.push(
                  this.state.exteriorImagesFileFirebase[i]
                )
              }
            }
                ...
                Object.keys(res.data).map(data => {
                  ...
                  axios
                    .put(
                      '/CarsList/' +
                        data +
                        '/ExteriorImages.json?auth=' +
                        this.props.token,
                      this.state.exteriorImages
                    )
                    .then(res => {
                      console.log(res)
      ...
      if (this.state.interiorImagesFile.length > 0) {
        ...
        if (i < this.state.interiorImagesFile.length) {
          if (this.state.interiorImagesFileFirebase.length !== 0) {
            for (
              let i = 0;
              i < this.state.interiorImagesFileFirebase.length;
              i++
            ) {
              if (this.state.interiorImagesFileFirebase[i] !== '') {
                this.state.interiorImages.push(
                  this.state.interiorImagesFileFirebase[i]
                )
              }
            }
                ...
                Object.keys(res.data).map(data => {
                  ...
                  axios
                    .put(
                      '/CarsList/' +
                        data +
                        '/InteriorImages.json?auth=' +
                        this.props.token,
                      this.state.interiorImages
                    )
                    .then(res => {
                      console.log(res)
  ...
  handleRemoveExteriorImage = index => {
    ...
    const newExteriorImages = this.props.exteriorImagesFileName.filter(
      (img, imgIndex) => {
  ...
  handleRemoveInteriorImage = index => {
    ...
    const newInteriorImages = this.props.interiorImagesFileName.filter(
      (img, imgIndex) => {
        return imgIndex !== index
      }
    )
  ...
  render() {
    ...
    return (
      <div>
        ...
        <Modal
          isOpen={this.state.editCarModalIsOpen}
          className="modal-dialog-centered"
          toggle={this.editCarToggleModal}
        >
          <ModalHeader
            style={{
              borderBottom: '0.5px solid rgba(169, 169, 169, .5)',
              height: '70px',
            }}
          ...
          >
            ...
            <input
              ...
              id="carNameEdit"
              style={{ display: 'block' }}
              value={this.props.carNameEdit}
              onChange={e => this.props.onSetNewCarName(e.target.value)}
            />
            ...
            <input
              ...
              id="pricePerdayEdit"
              style={{ display: 'block' }}
              value={this.props.pricePerdayEdit}
              onChange={e => this.props.onSetNewPricePerday(e.target.value)}
            />
            ...
            <input
              ...
              id="bodyTypeEdit"
              style={{ display: 'block' }}
              value={this.props.bodyTypeEdit}
              onChange={e => this.props.onSetNewBodyType(e.target.value)}
            />
            ...
            <input
              ...
              id="classEdit"
              style={{ display: 'block' }}
              value={this.props.classEdit}
              onChange={e => this.props.onSetNewClass(e.target.value)}
            />
            ...
            <input
              ...
              id="engineEdit"
              style={{ display: 'block' }}
              value={this.props.engineEdit}
              onChange={e => this.props.onSetNewEngine(e.target.value)}
            />
            ...
            <input
              ...
              id="horsePowerEdit"
              style={{ display: 'block' }}
              value={this.props.horsePowerEdit}
              onChange={e => this.props.onSetNewHorsePower(e.target.value)}
            />
            ...
            {Object.values(this.props.exteriorImagesFileName).map(
              (data, index) => {
                return (
                  <div key={index}>
                    <button
            ...
            {Object.values(this.props.interiorImagesFileName).map(
              (data, index) => {
                return (
                  <div key={index}>
                    <button
        ...
        <Modal
          isOpen={this.state.cancelEditingModalIsOpen}
          className="modal-dialog-centered"
          toggle={this.cancelEditButton}
        >
          <ModalHeader>
            <p className="modal-title" style={{ fontSize: '20px' }}>
          ...
          <ModalFooter>
            <button
              ...
              onClick={this.dontContinueWithoutSavingToggleModal}
            >
              No
            </button>
            <button
              ...
              onClick={this.continueWithoutSavingToggleModal}
            >
              Yes
            </button>
          </ModalFooter>
        ...
        <button
          className={['mr-1', classes.editCar].join(' ')}
          style={{
            outline: 'none',
...
const mapStateToProps = state => {
  return {
    ...
    exteriorImagesFileNameLength:
      state.editCarReducer.exteriorImagesFileNameLength,
    interiorImagesFileName: state.editCarReducer.interiorImagesFileName,
    interiorImagesFileNameLength:
      state.editCarReducer.interiorImagesFileNameLength,
...
const mapDispatchToProps = dispatch => {
  return {
    ...
    onEditInteriorImagesFileName: newInteriorImages =>
      dispatch(editCarActions.deleteInteriorImage(newInteriorImages)),
    onSetNewInteriorImagesFileNameLength: newInteriorImagesFileNameLength =>
      dispatch(
        editCarActions.newInteriorImagesFileNameLength(
          newInteriorImagesFileNameLength
        )
      ),
    onUpdateInteriorImagesArray: interiorImagesFileName =>
      dispatch(
        editCarActions.updateInteriorImagesInFirebase(interiorImagesFileName)
      ),
    onEditExteriorImagesFileName: newExteriorImages =>
      dispatch(editCarActions.deleteExteriorImage(newExteriorImages)),
    onSetNewExteriorImagesFileNameLength: newExteriorImagesFileNameLength =>
      dispatch(
        editCarActions.newExteriorImagesFileNameLength(
          newExteriorImagesFileNameLength
        )
      ),
    onUpdateExteriorImagesArray: exteriorImagesFileName =>
      dispatch(
        editCarActions.updateExteriorImagesInFirebase(exteriorImagesFileName)
      ),
    onSetNewCarName: newCarName =>
      dispatch(editCarActions.setNewCarName(newCarName)),
    onUpdateCarName: carName =>
      dispatch(editCarActions.updateCarNameInFirebase(carName)),
    onSetNewPricePerday: newPricePerday =>
      dispatch(editCarActions.setNewPricePerday(newPricePerday)),
    onUpdatePricePerday: pricePerday =>
      dispatch(editCarActions.updatePricePerdayInFirebase(pricePerday)),
    onSetNewBodyType: newBodyType =>
      dispatch(editCarActions.setNewBodyType(newBodyType)),
    onUpdateBodyType: bodyType =>
      dispatch(editCarActions.updateBodyTypeInFirebase(bodyType)),
    onSetNewClass: newClass => dispatch(editCarActions.setNewClass(newClass)),
    onUpdateClass: newClass =>
      dispatch(editCarActions.updateClassInFirebase(newClass)),
    onSetNewEngine: newEngine =>
      dispatch(editCarActions.setNewEngine(newEngine)),
    onUpdateEngine: engine =>
      dispatch(editCarActions.updateEngineInFirebase(engine)),
    onSetNewHorsePower: newHorsePower =>
      dispatch(editCarActions.setNewHorsePower(newHorsePower)),
    onUpdateHorsePower: horsePower =>
      dispatch(editCarActions.updateHorsePowerInFirebase(horsePower)),
  }
}

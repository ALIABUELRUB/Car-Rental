...
class History extends Component {
    ...
    componentDidMount = () => {
        ...
        this.props.onHistory(localStorage.getItem("isAdmin"), this.props.token, this.props.userId)
    }
                ...
                <div className={"table-responsive"} style={{ height: "400px" }}>
                        ...
                        <tbody>
                            {this.props.history.map((history, i) => {
                                return (<tr key={i}>
                                    ...
                                    <td style={{ textTransform: "capitalize" }}>{history.carName}</td>
                    ...
                    {this.props.history.length === 0 && !this.props.spinnerIsLoading ?
                        <p style={{ textAlign: "center" }}>
                            <strong>Empty History</strong></p> : null}
                </div>
            </div>
        );
    }
...
const mapDispatchToProps = dispatch => {
    return {
        onHistory: (isAdmin, token, userID) => dispatch(historyActions.history(isAdmin === "true", token, userID))
    }
}
